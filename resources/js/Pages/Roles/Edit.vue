...
const selectAllLists = () => {
    const listPermissions = Object.values(props.permissions)
        .flat()
        .filter(p => p.name.endsWith('.listar') || p.name.endsWith('.visualizar'))
        .map(p => p.name);
    // Preserva o que já tá marcado e adiciona os de listagem
    form.permissions = [...new Set([...form.permissions, ...listPermissions])];
};
                    ...
                    <form @submit.prevent="submit" class="space-y-6 max-w-4xl">
                        ...
                        <div>
                            ...
                            <TextInput
                                id="name"
                                type="text"
                                class="mt-1 block w-full max-w-md bg-gray-50"
                                v-model="form.name"
                                required
                                :disabled="role.name === 'Administrador'"
                            />
                            <p v-if="role.name === 'Administrador'" class="text-xs text-gray-500 mt-1">O nome do administrador é protegido pelo sistema.</p>
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" @click="selectAllLists" class="text-xs font-semibold text-emerald-600 hover:text-emerald-800 bg-emerald-50 px-2 py-1 rounded">Adicionar Listagens</button>
